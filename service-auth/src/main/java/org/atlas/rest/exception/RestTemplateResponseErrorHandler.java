    ...
    public void handleError(ClientHttpResponse response) throws IOException {
        ...
        if (statusCode.is4xxClientError()) {
            if (statusCode.equals(HttpStatus.CONFLICT)) {
                throw new UserAlreadyExistException(errorResponse.getMessage());
            } else if (statusCode.equals(HttpStatus.NOT_FOUND)) {
                throw new UsernameNotFoundException(errorResponse.getMessage());
            } else if (statusCode.equals(HttpStatus.BAD_REQUEST)) {
                throw new InvalidPasswordException(errorResponse.getMessage());
            }
